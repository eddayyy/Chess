...
{
	// Initialize the board with empty pieces (nullptr)
	for (int x = 0; x < 8; ++x) {
		for (int y = 0; y < 8; ++y) {
			_board[x][y] = nullptr;
		}
	...
	// Define the initial placement of the pieces
	...
	// Initialize each piece on the board
	...
	while (!quit)
	{
		while (SDL_PollEvent(&e))
		{
			...
			if (e.type == SDL_MOUSEBUTTONDOWN)
			{
				...
				{
					// Check if a piece exists at the clicked grid cell
			...
			else if (e.type == SDL_MOUSEBUTTONUP)
			{
				...
				{
					movePiece(draggingX, draggingY, gridX, gridY);
					setPosition(gridX, gridY, gridX * _SQUARE_SIZE, gridY * _SQUARE_SIZE); // Reset position

					// Reset dragging coordinates
					draggingX = -1;
					draggingY = -1;
				}
			}
		...
		for (int y = 0; y < _BOARD_SIZE; y++) {
			for (int x = 0; x < _BOARD_SIZE; x++) {
				if (_board[x][y]) {
					// Use piece's screen coordinates for rendering, if dragging
...
**  Helper functions
*******************************************************************************/
void BoardState::movePiece(int startX, int startY, int endX, int endY)
{
	if (_board[startX][startY] &&
		_board[startX][startY]->isLegalMove(startX, startY, endX, endY))
	{
